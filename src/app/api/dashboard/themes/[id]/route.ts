...
/**
 * DELETE /api/dashboard/themes/[id]
...
export async function DELETE(
  ...
  try {
    ...
    // Delete theme using RPC function (required for custom schema)
    // This function validates that theme belongs to account, is not active, and is not default
    ...
    if (deleteError) {
      // Check if it's a specific validation error from the function
      const errorMessage = deleteError.message || 'Failed to delete theme';
      
      // Handle specific error cases
      if (errorMessage.includes('active theme')) {
        return NextResponse.json(
      ...
      if (errorMessage.includes('default themes')) {
        return NextResponse.json(
      ...
      console.error('❌ [DEBUG] Delete theme error:', deleteError);
      return NextResponse.json(
        { error: errorMessage },
        { status: 500 }
      );
    ...
    // Supabase returns boolean directly or as array
    const isDeleted = Array.isArray(deleted) ? deleted[0] : deleted;
