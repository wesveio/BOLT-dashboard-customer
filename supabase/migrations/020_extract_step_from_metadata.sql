-- ============================================================================
-- EXTRACT STEP FROM METADATA IN INSERT ANALYTICS EVENTS FUNCTION
-- ============================================================================
-- This migration updates the insert_analytics_events function to extract
-- step from metadata if it's not at the top level of the event payload.
-- This fixes an issue where step was being stored in metadata from useEventTracker
-- but not being extracted to the step column.

-- Updated insert function that extracts step from metadata as fallback
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(
      NULLIF(event->>'step', 'null'),
      NULLIF(event->'metadata'->>'step', 'null')
    )::TEXT,
