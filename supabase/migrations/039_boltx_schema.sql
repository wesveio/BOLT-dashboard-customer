...
CREATE TABLE IF NOT EXISTS analytics.ai_insights (
  ...
  category TEXT NOT NULL CHECK (category IN ('revenue', 'conversion', 'ux', 'security')),
  title TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS analytics.ai_predictions (
  ...
  order_form_id TEXT,
  prediction_type TEXT NOT NULL DEFAULT 'abandonment' CHECK (prediction_type IN ('abandonment', 'conversion', 'friction')),
  ...
  intervention_type TEXT CHECK (intervention_type IN ('discount', 'security', 'simplify', 'progress')),
  metadata JSONB DEFAULT '{}'::jsonb,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS analytics.ai_optimizations (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  customer_id UUID NOT NULL REFERENCES customer.accounts(id) ON DELETE CASCADE,
  optimization_type TEXT NOT NULL CHECK (optimization_type IN ('form', 'layout', 'content', 'flow')),
  name TEXT NOT NULL,
  description TEXT,
...
CREATE TABLE IF NOT EXISTS analytics.ai_interventions (
  ...
  session_id TEXT NOT NULL,
  order_form_id TEXT,
  intervention_type TEXT NOT NULL CHECK (intervention_type IN ('discount', 'security', 'simplify', 'progress')),
  risk_score INTEGER NOT NULL CHECK (risk_score >= 0 AND risk_score <= 100),
  risk_level TEXT NOT NULL CHECK (risk_level IN ('low', 'medium', 'high', 'critical')),
  applied BOOLEAN DEFAULT FALSE,
  applied_at TIMESTAMPTZ,
  result TEXT CHECK (result IN ('converted', 'abandoned', 'pending')),
  metadata JSONB DEFAULT '{}'::jsonb,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
